...
api = KaggleApi()
api.authenticate()
data_path = 'F:\Personal_Projects\Diabetic_retinopathy\dataset'

# ------------- DOWNLOADING THE DATASET --------------------

# api.dataset_download_files('kushagratandon12/diabetic-retinopathy-aug')

#  ----------- Remove Some Data For Balacing The DataSet --------
# label = glob.glob('train/3/*.jpeg')
# label = list(label[5000::])
# print(len(label))
# for i in range(len(label)):
#     os.remove(label[i])

data_all_imgs = glob.glob(data_path+'\*\*\*.jpeg')
print(len(data_all_imgs))
IMG_SIZE = (512, 512)
